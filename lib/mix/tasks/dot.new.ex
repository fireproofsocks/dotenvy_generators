defmodule Mix.Tasks.Dot.New do
  use Mix.Task
  import DotenvyGenerators.Generator

  @shortdoc "Creates a new Elixir project using Dotenvy for configuration"

  @moduledoc """
  Creates a new Elixir project using Dotenvy for configuration.
  ...
  defp generate(app, mod, path, opts) do
    ...
    create_file("config/config.exs", parent_config_template(assigns))
    create_file("config/dev.exs", env_config_template([{:env, :dev} | assigns]))
    create_file("config/test.exs", env_config_template([{:env, :test} | assigns]))
    create_file("config/prod.exs", env_config_template([{:env, :prod} | assigns]))
    create_file("config/runtime.exs", runtime_config_template(assigns))

    create_directory("envs")
    create_file("envs/.env", env_template([{:env, "default"} | assigns]))
    create_file("envs/.dev.env", env_template([{:env, :dev} | assigns]))
    create_file("envs/.test.env", env_template([{:env, :test} | assigns]))
    create_file("envs/.prod.env", env_template([{:env, :prod} | assigns]))
  ...
  *.ez
  ...
  embed_template(:parent_config, ~S"""
  ...
  embed_template(:env_config, ~S"""
  # Compile-time configuration for @env environment
  import Config

  """)

  embed_template(:runtime_config, ~S"""
  ...
  # Sample configuration read from .envs inside envs/
  config :<%= @app %>, :secret, env!("SECRET", :string!)
  """)

  embed_template(:env, ~S"""
